...
type common struct {
	...
	isSynctest   bool
	weaveFailLog []byte // retained failure messages (synctest child under -weave), for weave reports
...
func (c *common) log(s string, isErr bool) {
	...
	// A synctest child's output is redirected to its parent (see destination), so
	// under -weave the child's own failure text would be lost to the weave report.
	// Retain error lines on the child itself (s is plain here, before framing) so
	// testingWeaveTest can surface the real reason.
	if isErr && c.isSynctest {
		c.mu.Lock()
		c.weaveFailLog = append(c.weaveFailLog, s...)
		c.mu.Unlock()
	}
...
// weaveTestFailure carries the accumulated log output of a test that failed via
// t.Error/Fatal (which run runtime.Goexit rather than panicking), so the weave
// engine reports the real reason instead of a generic sentinel. Its exported
// WeaveTestFailure method lets testing/synctest render it as a test failure
// rather than a Go panic.
type weaveTestFailure struct{ msg string }
...
func (c *common) weaveFailureText() string {
	c.mu.RLock()
	defer c.mu.RUnlock()
	return strings.TrimSpace(string(c.weaveFailLog))
}
...
func testingWeaveTest(t *T, f func(*T)) {
	...
	defer func() {
		...
		case !completed || t2.failed:
			...
			// reason, not a generic sentinel.
			msg := t2.weaveFailureText()
			if msg == "" {
				msg = "test failed"
			}
			panic(&weaveTestFailure{msg})
		}
	}()
